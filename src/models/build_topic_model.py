...
@click.command()
@click.argument('input_path', # path to the raw documents df (held as a pickle)
                type=click.Path(exists=True,
                                ...
                                resolve_path=True),)
def main(input_path):
    """ Read the raw documents dataframe and train a topic model from it.
    """
    logger = logging.getLogger(__name__)
    logger.info(f'reading {input_path}')

    if os.path.isdir(input_path):
        logger.info(f'{input_path} appears to be a directory; checking default filenames')
        df_path = PurePath(project_dir).joinpath(input_path).joinpath('raw_documents.pkl')
    else:
        df_path = input_path

    logger.info(f'loading dataframe from {df_path}')
    document_df = pd.read_pickle(df_path)
...
if __name__ == '__main__':
    ...
    log_path = PurePath(project_dir).joinpath('logs/topic_model_build.log')
